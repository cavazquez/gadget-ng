#!/usr/bin/env python3
"""analyze_conservation.py — Analiza conservación multi-step de los 40 runs Fase 5.

Para cada run bajo `runs/<tag>/`:
    1) Lee `diagnostics.jsonl` → serie temporal por paso (KE, p, L, COM).
    2) Lee los `frames/snap_XXXXXX/` → serie temporal por snapshot con PE total
       (vía `snapshot_metrics.potential_energy`), virial, half-mass radius.
    3) Combina ambas para obtener series completas de |ΔE/E|, |Δp|, |ΔL|.
    4) Lee `timings.json` → coste total y por paso.

Salidas:
    results/timeseries/<tag>.csv   (una serie por run)
    results/phase5_summary.csv     (resumen de los 40 runs)
...
# Parámetros físicos de los runs (deben coincidir con generate_configs.py).
SOFTENING = 0.05
G = 1.0


def parse_tag(tag: str) -> dict:
    """tag = '<dist>_N<N>_<variant>' → dict con componentes."""
    # Variantes son nombres con underscores; debemos romper por ese patrón.
    # Estrategia: localizar el segmento '_N<digits>_' y partir alrededor.
    for i, ch in enumerate(tag):
        if ch == "N" and i + 1 < len(tag) and tag[i - 1] == "_" and tag[i + 1].isdigit():
            break
    else:
        return {"dist": tag, "N": 0, "variant": ""}
    # tag[:i-1] = dist;  tag[i+1:] starts with digits then _<variant>
    dist = tag[: i - 1]
    rest = tag[i + 1:]
    # rest = <N>_<variant>
    n_end = 0
    ...
    variant = rest[n_end + 1:] if n_end < len(rest) else ""
    return {"dist": dist, "N": n_val, "variant": variant}
...
def load_diagnostics(path: Path) -> pd.DataFrame:
    ...
    # Normaliza columnas vectoriales a floats escalares
...
def analyze_run(run_dir: Path) -> tuple[pd.DataFrame, dict]:
    """Devuelve (df_timeseries, summary_row)."""
    tag = run_dir.name
    meta = parse_tag(tag)

    # ── Diagnostics (por paso) ───────────────────────────────────────────────
    ...
    # Drift vs step-0 (el step-0 es antes de cualquier integración)
    ...
    # Denominador robusto con max(|p0|, |p|)
    ...
    for fd in frame_dirs:
        ...
        # extraer step desde el nombre 'snap_XXXXXX'
...
def main() -> int:
    ...
    df = pd.DataFrame(summaries).sort_values(["N", "distribution", "variant"])
