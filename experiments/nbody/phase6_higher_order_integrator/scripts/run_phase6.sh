#!/usr/bin/env bash
# run_phase6.sh — Lanza los 12 runs dinámicos de la Fase 6.
#
# Cada run ejecuta `gadget-ng stepping` con uno de los 12 TOMLs generados por
# `generate_configs.py` y escribe salidas bajo `runs/<tag>/`. El script no
# re-ejecuta un run si su `timings.json` ya existe (idempotente): borra
# `runs/<tag>/` antes de relanzarlo si quieres forzar.
#
# Uso:
#   bash scripts/run_phase6.sh                    # todos los runs
#   bash scripts/run_phase6.sh plummer_a1         # filtra por prefijo
#   bash scripts/run_phase6.sh yoshida4           # filtra por integrador
#   PARALLEL=4 bash scripts/run_phase6.sh         # paraleliza
...
echo
echo "Fase 6 corrida completa. Runs en: $RUNS_DIR"
