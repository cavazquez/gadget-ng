#!/usr/bin/env python3
"""Phase 39 — aplicar `pk_correction` + comparar contra la referencia
`legacy` EH+CPT92 para un snapshot CLI evolucionado con un `dt` dado.

Mirror en Python de `crates/gadget-ng-analysis/src/pk_correction.rs` en el
modo `legacy` (único modo considerado por Phase 39):

```
P_ref(k, a) = P_EH(k, z=0) · [D(a)/D(a_init)]²
```

No se toca `pk_correction` ni la referencia; sólo se usa como
verificación cruzada CLI del pase `dt`.
"""
...
def main():
    ap = argparse.ArgumentParser(
        description="Phase 39: aplica pk_correction (Phase 35) a un snapshot "
                    "CLI y compara contra EH·[D(a)/D(a_init)]² (legacy).")
    ...
    ap.add_argument("--dt", type=float, required=True,
                    help="dt usado en la corrida CLI (metadata del sweep).")
    ...
    corrected = correct_pk(filt, args.box_internal, args.n, None)
    ...
    print(f"[phase39][dt={args.dt:.1e}] CLI evidence → {out_path}")
    print(f"    a                     : {args.a_snapshot}")
