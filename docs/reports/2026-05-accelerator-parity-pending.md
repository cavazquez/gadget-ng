...
AVX-512 Tree LET and CUDA MHD/Tree smoke-surface gaps.
...
| SPH cooling/dust/H2 | Complete | Complete | Rayon-backed, no dedicated AVX | Smoke/parity kernels |
| MHD induction/forces/cleaning | Complete | Complete | Rayon-backed, no dedicated AVX | Smoke/parity kernels |
| MHD anisotropic/Braginskii/reconnection/CR/dynamo | Complete | Complete | Rayon-backed, no dedicated AVX | Smoke/parity kernels |
| RT diagnostics/photoheating | Complete | Complete | Rayon-backed partial | Smoke/parity kernels |
| RT full M1 advection | Complete | Partial | Partial final-update vectorization | Pending |
| Analysis observables | Complete | Complete | Rayon-backed, no dedicated AVX | Pending |
...
| --- | --- | --- | --- | --- |
| AP-01 | Split `simd` and `rayon` Cargo features | Today many crates use `simd = ["dep:rayon"]`, so "SIMD without Rayon" is not a clean build axis outside explicit target-feature kernels. | Workspace supports serial, `rayon`, `explicit-simd`, and combined builds independently; tests cover all axes. | Pending |
...
Phase 201 should close AP-01 first. Separating `rayon` from explicit SIMD will make
the requested backend matrix testable instead of partly conventional.
