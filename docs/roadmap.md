...
2. **I/O**: **HDF5** (GADGET-like) y **bincode** opcionales en `gadget-ng-io` ✓; **lectura unificada** (`SnapshotReader` + `reader_for` + `read_snapshot_formatted` para JSONL/bincode/HDF5) ✓; **MessagePack** (`SnapshotFormat::Msgpack`, `--features msgpack`, `rmp-serde`, `particles.msgpack`) ✓; pendientes NetCDF.
