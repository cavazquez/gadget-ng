...
impl SpsGrid {
    ...
    pub fn bc03_lite() -> Self {
        ...
        Self { ages_gyr, metals, l_u, l_b, l_v, l_r, l_i }
    }
    ...
    pub fn interpolate(&self, age_gyr: f64, metallicity: f64, band: Spsband) -> f64 {
        let age_safe = age_gyr.max(self.ages_gyr[0]).min(*self.ages_gyr.last().unwrap());
        let z_safe = metallicity.max(self.metals[0]).min(*self.metals.last().unwrap());

        // Encontrar índices en la grilla
        let ia = self.ages_gyr.partition_point(|&a| a <= age_safe).saturating_sub(1)
            .min(self.ages_gyr.len() - 2);
        let iz = self.metals.partition_point(|&m| m <= z_safe).saturating_sub(1)
