...
fn analyse_lattice_no_halos_pk_finite() {
    ...
    assert_eq!(result.halos.len(), 0, "lattice uniforme no debe producir halos");
...
fn analyse_two_clusters_finds_two_halos() {
    ...
    let result = analyse(&particles, &params);
    assert_eq!(result.halos.len(), 2, "deben encontrarse exactamente 2 halos");
    // Halos ordenados por masa descendente; deben ser similares.
    let m0 = result.halos[0].mass;
    let m1 = result.halos[1].mass;
    assert!((m0 - m1).abs() / m0 < 0.05, "masas de ambos halos deben ser similares");
...
fn catalog_roundtrip_halos_and_pk() {
    ...
    let params = AnalysisParams {
        box_size: 1.0, b: 0.2, min_particles: 10, pk_mesh: 8, ..Default::default()
    };
    let result = analyse(&particles, &params);
