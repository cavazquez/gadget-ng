...
impl CudaMhdSolver {
    ...
    /// Aplica difusión óhmica resistiva en GPU (Phase 187).
    ///
    /// Implementa `dB/dt|_Ohm = −η_Ohm B / h²` (amortiguamiento local de B
    /// dependiente de la longitud de suavizado). La energía magnética disipada
    /// se deposita como calor: `Δu = heat_eff × ΔB² / (2 m)`.
    ///
    /// Réplica exacta de `gadget_ng_mhd::apply_ohmic_diffusion`.
    pub fn try_ohmic_diffusion(
        &self,
        particles: &mut [Particle],
        eta_ohm: f64,
        gamma: f64,
        ...
        #[cfg(cuda_unavailable)]
        {
            let _ = (particles, eta_ohm, gamma, dt);
            return Err(CudaExecutionError::Unavailable(CudaUnavailable {
                availability: CudaPmSolver::availability(),
            }));
        }
        ...
        #[cfg(not(cuda_unavailable))]
        {
            ...
            let mut by_out = vec![0.0_f32; n];
            let mut bz_out = vec![0.0_f32; n];
            // u_out receives the heat delta Δu = heat_eff * ΔB²/(2m) per particle.
            let mut u_delta_out = vec![0.0_f32; n];
            let heat_eff = (gamma - 1.0).max(0.0) as f32;

            // SAFETY: todos los slices son válidos y tienen longitud n.
            let code = unsafe {
                crate::ffi::cuda_mhd_ohmic(
                    soa_ptype.as_ptr(),
                    bx_in.as_ptr(),
                    by_in.as_ptr(),
                    bz_in.as_ptr(),
            ...
            check_kernel("cuda_mhd_ohmic", code)?;

            for (i, p) in particles.iter_mut().enumerate() {
                p.b_field.x = bx_out[i] as f64;
                p.b_field.y = by_out[i] as f64;
                p.b_field.z = bz_out[i] as f64;
                p.internal_energy += u_delta_out[i] as f64;
            }
            Ok(())
        }
    }
