...
use gadget_ng_core::{build_particles, IcKind, InitialConditionsSection, RunConfig, SimulationSection, Vec3};
...
fn distributed_tree_serial_kinetic_bounded() {
    let cfg = RunConfig {
        ...
        initial_conditions: InitialConditionsSection { kind: IcKind::Lattice },
    ...
    for _ in 0..cfg.simulation.num_steps {
        ...
        let x_lo_loc = parts.iter().map(|p| p.position.x).fold(f64::INFINITY, f64::min);
        let x_hi_loc = parts.iter().map(|p| p.position.x).fold(f64::NEG_INFINITY, f64::max);
