...
impl GpuBarnesHutFmm {
    ...
    fn create_buffers(
        ...
        let params_size = 48u64; // 12 × u32/f32
        let nodes_size = (n_nodes.max(1) as u64) * std::mem::size_of::<BhFmmGpuNode>() as u64;
        ...
        let hex_w_size = (n_nodes.max(1) * 15) as u64 * 4;
        ...
        let buf_hex_w = device.create_buffer(&wgpu::BufferDescriptor {
    ...
    pub fn compute_accelerations_raw(
        ...
        // SAFETY: `nodes` es un slice válido de `BhFmmGpuNode` (repr(C)).
        ...
        ctx.queue
            .write_buffer(&ctx.buf_hex_w, 0, &f32s_to_bytes(&hex_w_flat));
...
mod tests {
    ...
    fn try_new_does_not_panic() {
        let _result = GpuBarnesHutFmm::try_new();
    }

    #[test]
    fn two_particles_smoke_if_gpu_available() {
        let Some(solver) = GpuBarnesHutFmm::try_new() else {
            return;
        };
        ...
        let acc =
            solver.compute_accelerations_raw(&flat_pos, &flat_mass, &nodes, tree.root, &query, kp);
        assert_eq!(acc.len(), 6);
