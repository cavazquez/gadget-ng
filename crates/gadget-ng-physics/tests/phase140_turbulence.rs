...
fn gas(id: usize, vel: Vec3, b: Vec3) -> Particle {
    let mut p = Particle::new_gas(id, 1.0, Vec3::new(id as f64 * 0.1, 0.0, 0.0), vel, 1e8, 0.2);
    p.b_field = b;
    p
}
...
fn enabled_perturbs_velocities() {
    let mut particles: Vec<Particle> = (0..8)
        .map(|i| gas(i, Vec3::zero(), Vec3::new(1.0, 0.0, 0.0)))
        .collect();
    let cfg = turb_cfg(true, 1e-2);
    apply_turbulent_forcing(&mut particles, &cfg, 0.01, 1);
    let v_total: f64 = particles
        .iter()
        .map(|p| p.velocity.x.abs() + p.velocity.y.abs() + p.velocity.z.abs())
        .sum();
    assert!(
        v_total > 0.0,
        "El forzado debe perturbar velocidades: v_total={v_total:.4e}"
    );
}
...
fn stats_positive_mach_with_velocity() {
    ...
    assert!(
        alfven_mach > 0.0,
        "Alfvén Mach debe ser positivo: {alfven_mach}"
    );
}
