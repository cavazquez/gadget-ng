...
use gadget_ng_mhd::{
    C_LIGHT, advance_induction, apply_magnetic_forces, b_field_stats, dedner_cleaning_step,
...
fn power_spectrum_has_bins() {
    ...
    assert!(
        !pk.is_empty(),
        "P_B(k) debe retornar bins: got {} bins",
        pk.len()
    );
}
...
fn b_rms_nonzero_after_mhd_steps() {
    ...
    assert!(
        stats.b_rms > 0.0,
        "b_rms debe ser > 0 tras evolución: {:.3e}",
        stats.b_rms
    );
}
...
fn max_velocity_below_c() {
    ...
    assert!(
        v_max < C_LIGHT,
        "max|v| = {v_max:.3e} debe ser < C_LIGHT = {C_LIGHT:.3e}"
    );
}
...
fn power_spectrum_has_variation() {
    ...
    assert!(
        pk.len() >= 2,
        "Debe haber al menos 2 bins: got {}",
        pk.len()
    );
