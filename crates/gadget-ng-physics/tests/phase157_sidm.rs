...
fn kinetic_energy_conserved() {
    ...
    assert!(
        (ek_after - ek_before).abs() / ek_before.max(1e-30) < 1e-6,
        "Energía cinética no conservada: E_antes={:.6e}, E_después={:.6e}",
        ek_before,
        ek_after
    );
}
...
fn scatter_prob_grows_with_density() {
    ...
    assert!(
        p_high > p_low,
        "P_scatter debe crecer con densidad: {:.6e} vs {:.6e}",
        p_low,
        p_high
    );
}
...
fn sigma_m_zero_is_noop() {
    ...
    assert_eq!(
        particles[0].velocity.x, v0_before,
        "sigma_m=0 no debe cambiar velocidades"
    );
...
fn sidm_n20_moment_conserved() {
    let mut particles: Vec<Particle> = (0..20)
        .map(|i| dm_particle(i as f64 * 0.1, (i as f64 - 10.0) * 10.0, 0.3, 0.1))
        .collect();
    ...
    let px_after: f64 = particles.iter().map(|p| p.mass * p.velocity.x).sum();
    assert!(
        (px_after - px_before).abs() < 1e-4,
        "Momento total debe conservarse para N=20"
    );
}
