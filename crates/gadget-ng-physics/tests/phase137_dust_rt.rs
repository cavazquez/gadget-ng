...
use gadget_ng_rt::{radiation_gas_coupling_step_with_dust, M1Params, RadiationField};
...
fn tau_grows_with_dust_to_gas() {
    ...
    assert!(tau2 > tau1, "tau debe crecer con D/G: {tau1:.4e} vs {tau2:.4e}");
}
...
fn tau_grows_with_kappa() {
    ...
    let tau2 = dust_uv_opacity(1000.0, 0.01, 1.0, 0.1);
    assert!((tau2 / tau1 - 10.0).abs() < 1e-10, "tau ∝ kappa: ratio = {}", tau2/tau1);
}
...
fn attenuation_in_range() {
    for &dtg in &[0.0, 0.001, 0.005, 0.01] {
        ...
        let att = (-tau).exp();
        assert!(att >= 0.0 && att <= 1.0, "att={att:.4} fuera de [0,1] con D/G={dtg}");
    }
}
...
fn dust_section_kappa_default() {
    ...
    assert_eq!(cfg.kappa_dust_uv, 1000.0, "kappa_dust_uv default debe ser 1000");
}
...
fn coupling_with_dust_runs() {
    let mut particles: Vec<Particle> = (0..4).map(|i| {
        let mut p = Particle::new_gas(i, 1.0, Vec3::new(0.25 * i as f64, 0.5, 0.5),
                                     Vec3::zero(), 1e10, 0.2);
        p.dust_to_gas = 0.005;
        p
    }).collect();
    ...
    assert!(u_after >= u_before || u_after.is_finite(), "u debe ser finita");
}
