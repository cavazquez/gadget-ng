...
use gadget_ng_core::{
    CosmologyParams, FRParams, Particle, Vec3, apply_modified_gravity, chameleon_field,
    fifth_force_factor,
};
...
fn f_r0_zero_is_gr() {
    ...
    assert_eq!(
        particles[0].acceleration.x, acc_before,
        "f_r0=0 no debe cambiar la aceleración"
    );
}
...
fn fifth_force_factor_in_range() {
    ...
    assert!(
        ...
        "Factor de quinta fuerza debe estar en [0,1]: {}",
        factor
    );
}
...
fn screening_in_dense_region() {
    ...
    assert!(
        factor < 0.01,
        "En región densa, factor debe ser << 1 (screening), got {}",
        factor
    );
}
...
fn acceleration_nondecreasing_with_fr() {
    ...
    assert!(
        ...
        "La aceleración debe aumentar o mantenerse con f(R)"
    );
}
...
fn chameleon_field_decreasing() {
    ...
    assert!(
        f_low >= f_high,
        "Campo chameleon debe decrecer con densidad"
    );
}
...
fn modified_gravity_n100_no_panic() {
    ...
    let mut particles: Vec<Particle> = (0..100)
        .map(|i| dm_particle(i as f64 * 0.1, 1.0, 0.3, 0.1))
        .collect();
    apply_modified_gravity(&mut particles, &params, &cosmo, 1.0);
    for p in &particles {
