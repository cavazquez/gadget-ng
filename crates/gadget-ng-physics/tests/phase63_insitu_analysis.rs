...
fn skip() -> bool {
    std::env::var("PHASE63_SKIP").map(|v| v == "1").unwrap_or(false)
}
...
fn phase63_default_config_disabled() {
    ...
    assert!(!cfg.enabled, "por defecto el análisis in-situ debe estar desactivado");
...
fn phase63_interval_logic() {
    ...
    for step in [1u64, 2, 3, 4, 7, 8, 11, 13] {
        ...
        assert!(!should, "paso {step} no debe disparar análisis con interval=5");
    }
...
fn phase63_disabled_no_output() {
    ...
    let should_run = cfg.enabled && cfg.interval > 0 && (5u64 % cfg.interval == 0);
    assert!(!should_run, "con enabled=false no debe ejecutarse el análisis");
}
...
fn phase63_pk_finite_on_uniform() {
    ...
    let particles: Vec<Particle> = (0..n_side)
        .flat_map(|ix| (0..n_side).flat_map(move |iy| (0..n_side).map(move |iz| {
            (ix, iy, iz)
        })))
        .map(|(ix, iy, iz)| {
            let p = Particle::new(
    ...
    for bin in &result.power_spectrum {
        assert!(bin.pk.is_finite(), "P(k) debe ser finito: k={}, pk={}", bin.k, bin.pk);
