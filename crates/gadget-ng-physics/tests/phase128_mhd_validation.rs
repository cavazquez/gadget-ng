...
use gadget_ng_mhd::{advance_induction, apply_magnetic_forces, dedner_cleaning_step, magnetic_pressure, MU0};
...
fn alfven_wave_b_perp_conserved() {
    ...
    let b_perp_initial: Vec<f64> = particles.iter().map(|p| {
        (p.b_field.y.powi(2) + p.b_field.z.powi(2)).sqrt()
    }).collect();
    ...
    for (i, p) in particles.iter().enumerate() {
        ...
        assert!(rel_err < 0.5, "p{i}: |B_perp| cambió demasiado: {:.4} → {:.4}", b_perp_initial[i], b_perp);
    }
}
...
fn brio_wu_initial_pressure_jump() {
    ...
    assert!((p_b_left - p_b_right).abs() < 1e-12,
...
fn brio_wu_energy_finite() {
    ...
    assert!(e_mag.is_finite() && e_mag > 0.0, "Energía magnética debe ser finita: {e_mag}");
}
...
fn dedner_reduces_div_b() {
    ...
    let psi_max: f64 = particles.iter().map(|p| p.psi_div.abs()).fold(0.0_f64, f64::max);
    assert!(psi_max < 1.0, "ψ debe estar amortiguado: max|ψ| = {psi_max:.4}");
