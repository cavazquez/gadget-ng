...
use gadget_ng_core::{AgnSection, Particle, Vec3};
use gadget_ng_sph::{AgnParams, BlackHole, apply_agn_feedback_bimodal, bubble_feedback_radio};
...
fn gas_outside_bubble_not_affected() {
    ...
    assert_eq!(
        ...
        "Gas fuera de burbuja no cambia"
    );
}
...
fn quasar_mode_increases_internal_energy() {
    ...
    assert!(
        ...
        "modo quasar debe inyectar energía térmica"
    );
}
