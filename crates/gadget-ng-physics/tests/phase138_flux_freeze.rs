...
#[test]
fn zero_b_no_change() {
    let mut particles = vec![gas_with_b(0, 1.0, 0.3, 1.0, Vec3::zero())];
...
fn high_beta_applies_scale() {
    ...
    let mut particles = vec![gas_with_b(0, 1.0, h, u, Vec3::new(b0, 0.0, 0.0))];
    let rho = 1.0 / (h * h * h);
    ...
    assert!(
        ...
        "B debe aumentar por compresión: {:.4e} > {:.4e}",
        particles[0].b_field.x,
        b0
    );
}
...
fn low_beta_no_change() {
    ...
    let b_val = 1e6_f64;
    let mut particles = vec![gas_with_b(0, 1.0, 0.3, 1.0, Vec3::new(b_val, 0.0, 0.0))];
    let b_before = particles[0].b_field.x;
    let rho_ref = 1.0 / (0.3_f64 * 0.3 * 0.3);
    apply_flux_freeze(&mut particles, GAMMA, 100.0, rho_ref);
    assert_eq!(
        particles[0].b_field.x, b_before,
        "β bajo: B no debe cambiar"
    );
}
...
fn scale_follows_rho_power_2_3() {
    ...
    let mut particles = vec![gas_with_b(0, 1.0, h, 1e12, Vec3::new(b0, 0.0, 0.0))];
    apply_flux_freeze(&mut particles, GAMMA, 100.0, rho_ref);
...
fn mean_density_uniform() {
    let particles: Vec<Particle> = (0..5)
        .map(|i| gas_with_b(i, 1.0, 0.3, 1.0, Vec3::zero()))
        .collect();
    let rho_expected = 1.0 / (0.3_f64 * 0.3 * 0.3);
    let rho_actual = mean_gas_density(&particles);
    assert!(
        (rho_actual / rho_expected - 1.0).abs() < 1e-10,
