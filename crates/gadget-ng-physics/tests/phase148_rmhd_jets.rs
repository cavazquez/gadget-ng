...
use gadget_ng_mhd::{inject_relativistic_jet, lorentz_factor, C_LIGHT};
...
fn jet_injects_velocity() {
    ...
    assert!((particles[0].velocity.z - v_jet_expected).abs() < 1.0,
        "p0 vz = {:.3e} ≈ {v_jet_expected:.3e}", particles[0].velocity.z);
    assert!((particles[1].velocity.z + v_jet_expected).abs() < 1.0,
        "p1 vz = {:.3e} ≈ {:.3e}", particles[1].velocity.z, -v_jet_expected);
}
...
fn jet_aligns_b_with_velocity() {
    ...
    assert!(particles[0].b_field.z > 0.0, "B_jet debe apuntar en +z para p0");
    assert!(particles[1].b_field.z < 0.0, "B_jet debe apuntar en -z para p1");
}
...
fn jet_energy_relativistic() {
    ...
    assert!(particles[0].internal_energy >= u_expected * 0.9,
        "u_jet = {:.3e} ≥ 0.9 × {u_expected:.3e}", particles[0].internal_energy);
}
...
fn zero_v_jet_no_injection() {
    ...
    assert_eq!(particles[0].internal_energy, u_before, "v_jet=0 debe ser no-op");
}
