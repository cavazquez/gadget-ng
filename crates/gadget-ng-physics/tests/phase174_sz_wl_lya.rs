...
use gadget_ng_analysis::{
    compute_compton_y_map, compute_kinetic_sz_map, convergence_angular_cl,
    kaiser_squires_reconstruct, accumulate_tomographic_lensing,
    analyze_lya_forest, LyaCosmoParams, LyaParams, SzParams,
    LightconeHit, LensingMap, TomographyParams, compute_tau_along_sightline,
};
...
fn sz_compton_y_positive_for_hot_gas() {
    ...
    let params = SzParams { n_pixels: 64, axis: 'z' };
    let result = compute_compton_y_map(&particles, 100.0, &params, GAMMA);
    assert!(result.mean_y > 0.0, "mean_y > 0 for hot gas, got {}", result.mean_y);
}
...
fn sz_ksz_rms_positive_with_peculiar_velocity() {
    ...
    assert!(result.rms_ksz > 0.0, "rms_ksz > 0 with peculiar velocity, got {}", result.rms_ksz);
}
...
fn sz_compton_y_tiny_for_cold_gas() {
    ...
    assert!(result_cold.mean_y > result_hot.mean_y * 100.0,
        "Hot gas y should dominate cold gas: cold_y={} vs hot_y={}",
        result_hot.mean_y, result_cold.mean_y);
}
...
fn lya_analyze_returns_valid_structure() {
    ...
    assert!(result.mean_flux >= 0.0, "Mean flux should be non-negative, got {}", result.mean_flux);
}
...
fn lya_ionized_gas_transparent() {
    ...
    assert!(result.mean_flux > 0.9,
        "Ionized gas should have ⟨F⟩ > 0.9, got {}", result.mean_flux);
}

// T9: compute_tau_along_sightline gives absorption for neutral gas
#[test]
fn lya_single_sightline_neutral_absorption() {
    use gadget_ng_analysis::{compute_tau_along_sightline, LyaChemState};
    let cosmo = LyaCosmoParams::default();
    let params = LyaParams {
        n_sightlines: 4,
    ...
    let sightline = compute_tau_along_sightline(
        &particles, 'z', 50.0, 50.0, 100.0, &params, &cosmo, None,
    );
    // Neutral gas should produce some optical depth
    let max_tau = sightline.tau.iter().cloned().fold(0.0f64, f64::max);
assert!(max_tau > 0.0, "Neutral gas should produce τ > 0, got max_tau = {max_tau}");
}
