...
fn gas_with_te(id: usize, u: f64, t_e: f64) -> Particle {
    let mut p = Particle::new_gas(id, 1.0, Vec3::new(id as f64 * 0.1, 0.0, 0.0),
                                  Vec3::zero(), u, 0.2);
    p.t_electron = t_e;
    p
}
...
fn cfg_active() -> TwoFluidSection {
    TwoFluidSection { enabled: true, nu_ei_coeff: 10.0, t_e_init_k: 0.0 }
}
...
fn te_zero_initialized_to_ti() {
    ...
    assert!((t_e - t_i).abs() < t_i * 1e-10 || t_e == t_i,
...
fn te_always_non_negative() {
    ...
    for p in &particles {
        assert!(p.t_electron >= 0.0, "T_e debe ser ≥ 0: {:.3e}", p.t_electron);
    }
}
...
fn mean_te_over_ti_equilibrium() {
    ...
    let u = 1e12;
    let particles: Vec<Particle> = (0..10).map(|i| {
        let t_i = 2.0/3.0 * u;
        gas_with_te(i, u, t_i) // T_e = T_i exactamente
    }).collect();
    let ratio = mean_te_over_ti(&particles);
    assert!((ratio - 1.0).abs() < 1e-10, "T_e/T_i en equilibrio debe ser 1: {ratio:.6}");
}
...
fn non_gas_particles_ignored() {
    ...
    apply_electron_ion_coupling(&mut particles, &cfg, 0.1);
    assert_eq!(particles[0].t_electron, t_e_before, "Partículas DM deben ser ignoradas");
}
