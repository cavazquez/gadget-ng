...
use gadget_ng_core::{FeedbackSection, IsmSection, Particle, Vec3};
use gadget_ng_sph::{
    AgnParams, BlackHole, advance_stellar_ages, apply_agn_feedback_bimodal,
    apply_stellar_wind_feedback, diffuse_cr, inject_cr_from_sn, update_ism_phases,
};
...
fn ism_and_cr_independent() {
    ...
    assert!(
        (e_total_after_ism - u_before).abs() < 1e-9,
        "ISM debe conservar energía total"
    );
    ...
    assert_eq!(
        particles[0].internal_energy, u_thermal,
        "CR injection no cambia u térmica"
    );
...
fn disabled_modules_no_effect() {
    ...
    assert_eq!(
        particles[0].internal_energy, u_before,
        "ISM desactivado = no-op"
    );
    assert_eq!(
        particles[0].u_cold, 0.0,
        "ISM desactivado = no-op en u_cold"
    );
...
fn advance_ages_increments_stellar_age() {
    ...
    assert_eq!(
        particles[1].stellar_age, gas_age_before,
        "Gas no cambia edad estelar"
    );
}
...
fn full_baryonic_stack_no_panic() {
    ...
    let n = 50;
    let mut particles: Vec<Particle> = (0..n).map(|i| gas(i, 0.1 + (i as f64) * 0.01)).collect();
