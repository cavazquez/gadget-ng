...
fn dedner_maintains_finite_b() {
    ...
    for (i, p) in particles.iter().enumerate() {
        assert!(
            p.b_field.x.is_finite(),
            "B.x NaN tras 100 pasos Dedner en partícula {i}"
        );
        assert!(
            p.psi_div.is_finite(),
            "psi NaN tras 100 pasos Dedner en partícula {i}"
        );
