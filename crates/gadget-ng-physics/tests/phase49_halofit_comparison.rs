...
fn phase49_halofit_pk_at_ics_z49() {
    ...
    println!("  Mediana error P_sim/P_halofit = {:.1}%  (tolerancia 25 %)", med * 100.0);
...
fn phase49_halofit_linear_consistency() {
    ...
    println!(
        "[halofit_linear_consistency] D(z=2)/D(z=49) = {d_ratio:.4}  D² = {expected:.4}"
    );
...
fn phase49_halofit_ratio_z2_vs_z0() {
    ...
    for ki in 1..k_nl.len() {  // ki=1 (k=1 h/Mpc), ki=2 (k=2 h/Mpc)
    ...
    assert!(boosts[0][0] >= boosts[3][0], "Boost k=0.3 no decrece de z=0 a z=5");

    println!(
        "  OK — Boost(k=1 h/Mpc, z=0) = {boost_k1_z0:.3} > 2.0  ✓"
    );
}
