...
fn w0_minus1_is_lcdm() {
    ...
    assert!(
        (w + 1.0).abs() < 1e-12,
        "w(a=1, w0=-1, wa=0) debe ser -1, got {}",
        w
    );
}
...
fn advance_a_stable_1000_steps() {
    ...
    for _ in 0..1000 {
        ...
        assert!(
            a.is_finite() && a > 0.0,
            "factor de escala debe permanecer positivo y finito"
        );
    }
}
...
fn w_varies_with_a() {
    ...
    assert!(
        (w_a1 - w_a05).abs() > 1e-6,
        "w(a) debe variar con a para wa != 0"
    );
}
...
fn hubble_in_physical_limits() {
    ...
    for i in 1..10 {
        ...
        assert!(
            h.is_finite() && h >= 0.0,
            "H(a={:.1}) debe ser finito y no negativo, got {}",
            a,
            h
        );
    }
