...
use gadget_ng_io::hdf5_parallel_writer::{
    Hdf5ParallelOptions, read_snapshot_hdf5_serial, write_snapshot_hdf5_serial,
};

fn skip_hdf5() -> bool {
    // Si el binario no tiene hdf5 habilitado, las funciones devuelven Err.
    // Los tests detectan esto y pasan silenciosamente.
    std::env::var("PHASE65_SKIP")
        .map(|v| v == "1")
        .unwrap_or(false)
}
...
fn phase65_parallel_write_read_p1() {
    ...
    assert!(
        (snap.box_size - 10.0).abs() < 1e-5,
        "box_size debe ser ~10.0"
    );
    ...
    for (orig, read) in particles.iter().zip(snap.particles.iter()) {
        ...
        let dm = (orig.mass - read.mass).abs();
        assert!(
            dx < 1e-4,
            "posición x diferente: orig={}, read={}",
            orig.position.x,
            read.position.x
        );
        assert!(
            dm < 1e-4,
            "masa diferente: orig={}, read={}",
            orig.mass,
            read.mass
        );
    }
