...
use gadget_ng_mhd::{
    C_LIGHT, advance_srmhd, em_energy_density, lorentz_factor, srmhd_conserved_to_primitive,
};
...
fn primitive_rest_no_b() {
    ...
    assert!(
        (rho - rho0).abs() / rho0 < 0.1,
        "ρ = {rho:.4}, esperado {rho0}"
    );
...
fn srmhd_no_effect_below_threshold() {
    ...
    assert_eq!(
        particles[0].position.x, pos_before.x,
