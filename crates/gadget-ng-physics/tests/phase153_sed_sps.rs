...
use gadget_ng_analysis::sps_tables::{SpsGrid, Spsband, sps_luminosity};
...
fn lb_decreases_with_age() {
    ...
    assert!(
        l_b_young > l_b_old,
        "L_B debe decrecer con edad: joven={}, viejo={}",
        l_b_young,
        l_b_old
    );
}
...
fn bv_reddens_with_age() {
    ...
    assert!(
        bv_old > bv_young,
        "B-V debe enrojecerse con la edad: joven={:.3}, viejo={:.3}",
        bv_young,
        bv_old
    );
}
...
fn higher_metallicity_more_luminous() {
    ...
    assert!(
        l_b_high_z > l_b_low_z,
        "Metalicidad alta debe dar más luminosidad: low={}, high={}",
        l_b_low_z,
        l_b_high_z
    );
}
...
fn bilinear_interpolation_on_node() {
    ...
    assert_eq!(
        v_exact, grid.l_v[0][0],
        "Interpolación debe coincidir en nodo de la tabla"
    );
}
...
fn galaxy_sed_returns_valid_result() {
    let particles = vec![star_particle(1.0, 0.02, 1.0), star_particle(5.0, 0.02, 1.0)];
    let sed = galaxy_sed(&particles);
    assert!(
        sed.l_b > 0.0 && sed.l_v > 0.0,
        "L_B y L_V deben ser positivas"
    );
