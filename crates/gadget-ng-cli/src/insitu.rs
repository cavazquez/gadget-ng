...
pub fn maybe_run_insitu(
    ...
    let out_dir = cfg.output_dir.as_deref().unwrap_or(default_out_dir);
...

    {
        ...
        let assembly_bias_out: Option<AssemblyBiasOut> =
            if cfg.assembly_bias_enabled && result.halos.len() >= 4 {
                ...
                let spins: Vec<f64> = result.halos.iter().map(|h| h.velocity_dispersion).collect();
        ...
        let cm21_out = if cfg.cm21_enabled {
            ...
            if gas_indices.is_empty() {
                ...
                let gas_particles: Vec<Particle> =
                    gas_indices.iter().map(|&i| particles[i].clone()).collect();
                // Usar ChemState reales si están disponibles (EoR acoplado)
                let chem_for_21cm: Vec<gadget_ng_rt::ChemState> =
                    if let Some(chem) = chem_states_opt {
                        gas_indices
                            .iter()
                            .map(|&i| {
                                if i < chem.len() {
