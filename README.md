...
**Paridad GPU completa (AP-01 → AP-20):** **AP-03** — validación hardware GTX 1060 (sm_61, CUDA 12.4). **AP-07** — LET GPU mono+quad+oct (max\_rel=9.3e-7, 14–778× speedup). **AP-18** — SPH density+Balsara+forces CUDA, Tree LET. **AP-19** — pipeline SPH CUDA persistente (−65% PCIe, break-even N≈120). **Phase 186** — MHD Hall term CPU+SIMD+CUDA (Rodrigues). **AP-20** — cierre de los 4 gaps finales: (A) `mhd_hall_drift_kernel` GPU (Rodrigues por hilo, `|ΔB|<1e-4`); (B) `fr_screening_per_cell_kernel` + `fr_screening_jacobi_kernel` GPU (L2<1e-3); (C) `treepm_sr_erfc_kernel` O(N²) con erfc A&S + mínima imagen (<5%); (D) `bh_walk_monopole_kernel` con pila por hilo stack[32] + MAC θ² (<1%). Todos los gaps remanentes documentados en AP-19 están cerrados. Ver [`docs/reports/2026-05-accelerator-parity-pending.md`](docs/reports/2026-05-accelerator-parity-pending.md).
