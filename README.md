...
[![🦀 Rust](https://img.shields.io/badge/🦀_Rust-1.95%2B_%28edition_2024%29-orange?logo=rust&logoColor=white)](https://www.rust-lang.org/)
...
[![📈 Coverage](https://codecov.io/gh/cavazquez/gadget-ng/branch/main/graph/badge.svg)](https://codecov.io/gh/cavazquez/gadget-ng)
...
| Ámbito | Herramientas |
|---|---|
| 🦀 **Core** | Rust 1.95+ (edición 2024), workspace multi-crate, `serde`, `toml`, `clap`, `rayon` |
...
| **MHD Ohmic diffusion** | `apply_ohmic_diffusion` — `dB/dt = −η_Ohm B/h²`; amortiguamiento local + calor disipado; dispatch AVX2/AVX512/Rayon+CUDA; opt-in `[mhd] ohmic_enabled = true, ohmic_eta = ...` (Phase 187) |
| **MHD ambipolar + química** | `apply_ambipolar_diffusion_with_chem` — usa `x_e` real del solver de química (ChemState) en lugar del proxy térmico; acopla MHD no-ideal a RT/química; opt-in `ambipolar_use_chem_ionization = true` (Phase 187) |
...
| Barnes-Hut / Tree local | ✅ | ✅ | ✅ AVX2 + AVX512 local walk | ✅ opt-in `cuda_tree` — `bh_walk_monopole_kernel` con pila por hilo (stack[32]), MAC θ², auto-skip; wired en `compute_forces_local_tree` (AP-20) |
| Tree LET / RMN SoA | ✅ | ✅ | ✅ AVX2 + AVX512 | ✅ opt-in `cuda_tree` — `try_tree_walk_let` (mono+quad+oct); wired en flat LET path (AP-07) |
| TreePM corto alcance | ✅ | ✅ | ✅ AVX2 + AVX512 SR kernel | ✅ opt-in `cuda_tree` — `treepm_sr_erfc_kernel` O(N²) con erfc A&S §7.1.26 + mínima imagen; wired en SR paso SFC (AP-20) |
| PM CIC assign/interp | ✅ | ✅ | ✅ AVX2 + AVX512 | ✅ ⚡ |
| PM FFT/Poisson | ✅ | ✅ k-space + PM path | ✅ AVX2 + AVX512 spectral kernel | ✅ ⚡ |
| SPH density | ✅ | ✅ | ✅ Wendland AVX2 + AVX512 batch — Rayon exterior + `w_and_grad_w_batch` SIMD runtime (AP-21: ya combinado, documentado + parity test) | ✅ opt-in `cuda_sph` — `cuda_sph_density`; wired en `step_sph` KDK |
| SPH forces Gadget-2/Balsara | ✅ | ✅ | ✅ Wendland AVX2 + AVX512 batch — AP-21: `grad_w_batch` SIMD sobre buffer de vecinos en `sph_gadget2_update_for_particle` (3 fases: collect → batch-SIMD h_i → accumulate) | ✅ opt-in `cuda_sph` — pipeline persistente AP-19: `cuda_sph_density` → `cuda_sph_balsara` → `cuda_sph_gadget2_forces`; 1 upload H→D (−65% PCIe vs AP-18); wired en `step_sph` KDK |
...
| MHD CR streaming / dynamo | ✅ | ✅ | ✅ AVX2 + AVX512 streaming local update + dynamo — AP-21: `streaming_crk_par` (Rayon exterior, O(N²) paralelo) + `streaming_crk_par_simd`; dispatcher triple via | ✅ opt-in `[accelerators] cuda_cr = true` — `mhd_cr_streaming_o2_kernel` + `mhd_cr_backreaction_kernel`; wired en `step_sph` |
| MHD Hall term (nonideal) | ✅ | ✅ | ✅ AVX2 + AVX512 Rodrigues rotation, conserva \|B\|, sin heating | ✅ opt-in `cuda_mhd` — `mhd_hall_drift_kernel`; Rodrigues por hilo; wired en `step_mhd` (AP-20) |
...
| MHD ambipolar + química (nonideal) | ✅ | ✅ | — O(N) lineal, no requiere SIMD batch | — CPU Rayon O(N) suficiente; `step_mhd_chem_ambipolar` (Phase 187) |
| MHD Ohmic diffusion (nonideal) | ✅ | ✅ | ✅ AVX2 + AVX512 damping `exp(−η dt/h²)` + Δu heat; 4-lane/8-lane loops | ✅ opt-in `cuda_mhd` — `mhd_ohmic_kernel`; B damping + Δu por hilo; wired en `step_mhd` (Phase 187) |
...
| RT chemistry stiff solver | ✅ | ✅ | ✅ AVX2 + AVX512 masked-lane dispatch — AP-21: `apply_chemistry_par_simd` — Rayon por chunks de 64 + SIMD batch (photoionización, solver implícito, cooling); dispatcher triple via; parity test 1e-12 | ✅ opt-in `[accelerators] cuda_rt_chem = true` — `rt_chemistry_stiff_kernel`; wired en `step_reionization` |
...
| f(R) / modified gravity PM | ✅ | ✅ via PM path | ✅ PM spectral path | ✅ `CudaPmSolver::try_fr_screening_field` — `fr_screening_per_cell_kernel` + `fr_screening_jacobi_kernel`; `PmSolver::screening_override` inyectable desde CLI (AP-20) |
...
Leyenda: ✅ implementado y wired opt-in; ⚠️ kernel CPU completo, CUDA parcial o pendiente; ❌ sin kernel CUDA. Todos los gaps principales cerrados en AP-20.
⚡ = buffers persistentes `CudaPool` (AP-02): sin `cudaMalloc`/`cudaFree` por paso; redimensionamiento automático por duplicación.

**Paridad GPU completa (AP-01 → Phase 187):** **AP-03** — validación hardware GTX 1060 (sm_61, CUDA 12.4). **AP-07** — LET GPU mono+quad+oct (max\_rel=9.3e-7, 14–778× speedup). **AP-18** — SPH density+Balsara+forces CUDA, Tree LET. **AP-19** — pipeline SPH CUDA persistente (−65% PCIe, break-even N≈120). **Phase 186** — MHD Hall term CPU+SIMD+CUDA (Rodrigues). **AP-20** — cierre de los 4 gaps finales: (A) `mhd_hall_drift_kernel` GPU; (B) `fr_screening_per_cell/jacobi_kernel` GPU; (C) `treepm_sr_erfc_kernel`; (D) `bh_walk_monopole_kernel`. **Phase 187** — MHD Ohmic diffusion `mhd_ohmic_kernel` CUDA (`exp(−η dt/h²)` + Δu por hilo) + ambipolar acoplada a química CPU. Ver [`docs/reports/2026-05-accelerator-parity-pending.md`](docs/reports/2026-05-accelerator-parity-pending.md).
