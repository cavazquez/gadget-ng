...
| SPH forces Gadget-2/Balsara | ✅ | ✅ | ✅ Wendland AVX2 + AVX512 batch | ✅ opt-in `cuda_sph` — pipeline persistente AP-19: `cuda_sph_density` → `cuda_sph_balsara` → `cuda_sph_gadget2_forces`; 1 upload H→D (−65% PCIe vs AP-18); wired en `step_sph` KDK |
